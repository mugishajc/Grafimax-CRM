                ...
                <div class="card">
                    <div class="card-header">
                        <div class="d-flex justify-content-between w-100">
                            ...
                            @can('create user')
                                <a href="#" data-url="{{ route('users.create') }}" data-ajax-popup="true" data-title="{{__('Create New User')}}" class="btn btn-sm btn-warning">
                                        ...
                                        <div class="staff staff-grid-view pb-0">
                                            ...
                                            <div class="meta-info mb-3">
                                                ...
                                                @if($user->delete_status==0)
                                                    <p class="soft-del text-danger">{{__('Soft Deleted')}}</p>
                                                @endif
                                            </div>
                                            <div class="btn-wrap">
                                                @if(\Auth::user()->type=='super admin')
                                                    <button class="btn" title="User">
                                                        ...
                                                        <span class="pl-2">{{$user->total_company_user($user->id)}} </span>
                                                    </button>
                                                    <button class="btn" title="Project">
                                                        <i class="fas fa-file-invoice-dollar"></i>
                                                        <span class="pl-2">{{$user->total_company_project($user->id)}}</span>
                                                    </button>
                                                    <button class="btn" title="Client">
                                                        <i class="fas fa-tasks"></i>
                                                        <span class="pl-2">{{$user->total_company_client($user->id)}}</span>
                                                    </button>
                                                @else
                                                    <button class="btn" title="Project">
                                                        <i class="fas fa-briefcase"></i>
                                                        <span class="pl-2">{{$user->user_project()}} </span>
                                                    </button>
                                                    <button class="btn" title="Expense">
                                                        <i class="fas fa-file-invoice-dollar"></i>
                                                        <span class="pl-2">{{\Auth::user()->priceFormat($user ->user_expense())}}</span>
                                                    </button>
                                                    <button class="btn" title="Task">
                                                        <i class="fas fa-tasks"></i>
                                                        <span class="pl-2">{{$user->user_assign_task()}}</span>
                                                    </button>
                                                @endif
                                            </div>
                                        </div>
