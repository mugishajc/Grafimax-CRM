...
class Invoicr extends \FPDF_rotation
{
    ...
    function Header()
    {
        ...
        $this->Cell(0, 5, iconv("UTF-8", "UTF-8", strtoupper($this->title)), 0, 1, 'R');
        ...
        $this->Cell(32, $lineheight, iconv("UTF-8", "UTF-8", strtoupper($this->l['number']) . ':'), 0, 0, 'L');
        ...
        $this->SetFont($this->font, 'B', 9);
        $this->SetTextColor($this->color[0], $this->color[1], $this->color[2]);
        $this->Cell(32, $lineheight, iconv("UTF-8", "UTF-8", strtoupper($this->l['date'])) . ':', 0, 0, 'L');
        $this->SetTextColor(50, 50, 50);
        $this->SetFont($this->font, '', 9);
        ...
        if($this->due)
        {
            ...
            $this->SetFont($this->font, 'B', 9);
            $this->SetTextColor($this->color[0], $this->color[1], $this->color[2]);
            $this->Cell(32, $lineheight, iconv("UTF-8", "UTF-8", strtoupper($this->l['due'])) . ':', 0, 0, 'L');
            $this->SetTextColor(50, 50, 50);
            $this->SetFont($this->font, '', 9);
        ...
        if($this->PageNo() == 1)
        {
            ...
            if(is_array($this->from) && is_array($this->to))
            {
                for($i = 1; $i < max(count($this->from), count($this->to)); $i++)
                {
                    $this->Cell($width, $lineheight, iconv("UTF-8", "UTF-8", $this->from[$i]), 0, 0, 'L');
                    $this->Cell(0, $lineheight, iconv("UTF-8", "UTF-8", $this->to[$i]), 0, 0, 'L');
                    $this->Ln(5);
                }
                $this->Ln(-6);
            }
        ...
        {
            if(isset($this->firstColumnWidth))
            {
                ...
                $this->Cell($this->firstColumnWidth, 10, iconv("UTF-8", "UTF-8", strtoupper($this->l['product'])), 0, 0, 'L', 0);
                $this->Cell($this->columnSpacing, 10, '', 0, 0, 'L', 0);
                $this->Cell($width_other, 10, iconv("UTF-8", "UTF-8", ''), 0, 0, 'C', 0);
                //                $this->Cell($this->columnSpacing, 10, '', 0, 0, 'L', 0);
                //                $this->Cell($width_other, 10, iconv("UTF-8", "UTF-8", strtoupper($this->l['vat'])), 0, 0, 'C', 0);
                //                $this->Cell($this->columnSpacing, 10, '', 0, 0, 'L', 0);
                //                $this->Cell($width_other, 10, iconv("UTF-8", "UTF-8", strtoupper($this->l['price'])), 0, 0, 'C', 0);
                //                if(isset($this->discountField))
                //                {
                //                    $this->Cell($this->columnSpacing, 10, '', 0, 0, 'L', 0);
                //                    $this->Cell($width_other, 10, iconv("UTF-8", "UTF-8", strtoupper($this->l['discount'])), 0, 0, 'C', 0);
                //                }
                $this->Cell($this->columnSpacing, 10, '', 0, 0, 'L', 0);
                $this->Cell($width_other, 10, iconv("UTF-8", "UTF-8", strtoupper($this->l['total'])), 0, 0, 'C', 0);
                $this->Ln();
                $this->SetLineWidth(0.3);
                $this->SetDrawColor($this->color[0], $this->color[1], $this->color[2]);
    ...
    function Body()
    {
        ...
        if($this->items)
        {
            foreach($this->items as $item)
            {
                if($item['description'])
                {
                    ...
                    $calculateHeight->MultiCell($this->firstColumnWidth, 3, iconv("UTF-8", "UTF-8", $item['description']), 0, 'L', 1);
                ...
                $x = $this->GetX();
                $this->Cell($this->firstColumnWidth, $cHeight, iconv("UTF-8", "UTF-8", $item['item']), 0, 0, 'L', 1);
                if($item['description'])
                {
                    $resetX = $this->GetX();
                    $resetY = $this->GetY();
                    ...
                    $this->MultiCell($this->firstColumnWidth, 3, iconv("UTF-8", "UTF-8", $item['description']), 0, 'L', 1);
                    //Calculate Height
                    $newY    = $this->GetY();
        ...
        foreach($this->addText as $text)
        {
            if($text[0] == 'title')
            {
                $this->SetFont($this->font, 'b', 9);
                $this->SetTextColor(50, 50, 50);
                $this->Cell(0, 10, iconv("UTF-8", "UTF-8", strtoupper($text[1])), 0, 0, 'L', 0);
                $this->Ln();
                $this->SetLineWidth(0.3);
                $this->SetDrawColor($this->color[0], $this->color[1], $this->color[2]);
                $this->Line($this->margins['l'], $this->GetY(), $this->document['w'] - $this->margins['r'], $this->GetY());
            ...
            if($text[0] == 'paragraph')
            {
                ...
                $this->SetFont($this->font, '', 8);
                $this->MultiCell(0, 4, iconv("UTF-8", "UTF-8", $text[1]), 0, 'L', 0);
                $this->Ln(4);
            }
        }
    }
    ...
    private function setLanguage($language)
    {
        ...
        $l             = [];
        $l['number']   = __('Reference');
        $l['date']     = __('Billing date');
        $l['due']      = __('Due date');
        $l['to']       = __('Billing to');
        $l['from']     = __('Our information');
        $l['product']  = __('Product');
        $l['amount']   = __('Amount');
        $l['price']    = __('Price');
        $l['discount'] = __('Discount');
        $l['vat']      = __('Vat');
        $l['total']    = __('Total');
        $l['page']     = __('Page');
        $l['page_of']  = __('of');
